...
type CashReseanType = "checkFori" | "buyGoods";

type Props = {
  value: string;
  onChange: (value: CashReseanType) => void;
  className?: string;
};

const CashReseanComponent: React.FC<Props> = ({
  value,
  onChange,
  className = "",
}) => {
  ...
  const cashResean = [
    { value: "checkFori" as CashReseanType, label: " بابت چک برگشتی" },
    { value: "buyGoods" as CashReseanType, label: " بابت خرید کالا" },
  ];

  const selectedType = cashResean.find((type) => type.value === value);
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      >
        <span className="text-right">
          {selectedType?.label || "انتخاب دلیل پرداخت"}
        </span>
        <svg
...
export default CashReseanComponent;
